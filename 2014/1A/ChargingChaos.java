...
public class ChargingChaos {
	...
	public static void main(String[] args) {
		...
		for (int caseNo = 1; caseNo <= T; caseNo++)
		{
			...
			{
				BitSet flips = q.poll();
				if (minCardinality < flips.cardinality())
				{
					visited.clear();
				}
				...
				for (int i = flips.nextClearBit(0); i >= 0; i = flips.nextClearBit(i+1)) {
					BitSet newFlip = (BitSet) flips.clone();
					newFlip.set(i, true);
					if (newFlip.cardinality() >= minCardinality && !visited.contains(newFlip))
					{
						visited.add(newFlip);
						q.add(newFlip);
					}
